	...
	if (Server()->ClientIngame(ClientID))
	{
		...
		else if (MsgID == NETMSGTYPE_CL_VOTE)
		{
			...
			else if (m_apPlayers[ClientID] && m_apPlayers[ClientID]->m_PlayerFlags & PLAYERFLAG_SCOREBOARD)
			{
				...
				if (ScoreMode < 0)
					ScoreMode = NB_PLAYERSCOREMODE - 1;
				if (ScoreMode >= NB_PLAYERSCOREMODE)
					ScoreMode = 0;
...
bool CGameContext::ConSetClass(IConsole::IResult *pResult, void *pUserData)
{
	...
	if (str_comp(pClassName, "engineer") == 0)
		pPlayer->SetClass(PLAYERCLASS_ENGINEER);
	else if (str_comp(pClassName, "soldier") == 0)
		pPlayer->SetClass(PLAYERCLASS_SOLDIER);
	else if (str_comp(pClassName, "scientist") == 0)
		pPlayer->SetClass(PLAYERCLASS_SCIENTIST);
	else if (str_comp(pClassName, "biologist") == 0)
	...
	else if (str_comp(pClassName, "magician") == 0)
		pPlayer->SetClass(PLAYERCLASS_MAGICIAN);
	else if (str_comp(pClassName, "looper") == 0)
		pPlayer->SetClass(PLAYERCLASS_LOOPER);
	else if (str_comp(pClassName, "police") == 0)
		pPlayer->SetClass(PLAYERCLASS_POLICE);
	else if (str_comp(pClassName, "medic") == 0)
		pPlayer->SetClass(PLAYERCLASS_MEDIC);
	else if (str_comp(pClassName, "hero") == 0)
		pPlayer->SetClass(PLAYERCLASS_HERO);
	else if (str_comp(pClassName, "ninja") == 0)
		pPlayer->SetClass(PLAYERCLASS_NINJA);
	else if (str_comp(pClassName, "mercenary") == 0)
		...
		pPlayer->SetClass(PLAYERCLASS_HUNTER);
	else if (str_comp(pClassName, "bat") == 0)
		pPlayer->SetClass(PLAYERCLASS_BAT);
	else if (str_comp(pClassName, "boomer") == 0)
		pPlayer->SetClass(PLAYERCLASS_BOOMER);
	else if (str_comp(pClassName, "ghost") == 0)
		pPlayer->SetClass(PLAYERCLASS_GHOST);
	else if (str_comp(pClassName, "spider") == 0)
		pPlayer->SetClass(PLAYERCLASS_SPIDER);
	else if (str_comp(pClassName, "ghoul") == 0)
		pPlayer->SetClass(PLAYERCLASS_GHOUL);
	else if (str_comp(pClassName, "slug") == 0)
		pPlayer->SetClass(PLAYERCLASS_SLUG);
	else if (str_comp(pClassName, "slime") == 0)
		pPlayer->SetClass(PLAYERCLASS_SLIME);
	else if (str_comp(pClassName, "voodoo") == 0)
		pPlayer->SetClass(PLAYERCLASS_VOODOO);
	else if (str_comp(pClassName, "undead") == 0)
		pPlayer->SetClass(PLAYERCLASS_UNDEAD);
	else if (str_comp(pClassName, "witch") == 0)
		pPlayer->SetClass(PLAYERCLASS_WITCH);
	else if (str_comp(pClassName, "freezer") == 0)
...
bool CGameContext::ConChatInfo(IConsole::IResult *pResult, void *pUserData)
{
	...
	Buffer.append("\n\n");
	pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Mod version:{str:ModVersion}, Version code:{str:VersionCode}"), "ModVersion", MOD_VERSION, "VersionCode", VERSION_CODE, NULL);
	Buffer.append("\n\n");
...
bool CGameContext::PrivateMessage(const char *pStr, int ClientID, bool TeamChat)
{
	...
	for (; c < sizeof(aNameFound) - 1; c++)
	{
		if (pStr[c] == ' ' || pStr[c] == 0)
		{
			...
			else if (str_comp(aNameFound, "!slime") == 0 && m_apPlayers[ClientID] && m_apPlayers[ClientID]->GetCharacter())
			{
...
bool CGameContext::ConHelp(IConsole::IResult *pResult, void *pUserData)
{
	...
	if (pHelpPage)
	{
		if (str_comp_nocase(pHelpPage, "game") == 0)
		{
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The goal for humans is to survive until the army cleans the map."), NULL);
			Buffer.append("\n\n");
		...
		else if (str_comp_nocase(pHelpPage, "engineer") == 0)
		{
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("When an infected touches the wall, he dies."), NULL);
			Buffer.append("\n\n");
		...
		else if (str_comp_nocase(pHelpPage, "soldier") == 0)
		{
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(
				Buffer, pLanguage,
				("Bomb default can explode {int:NumBombs} times."),
				"NumBombs", &g_Config.m_InfSoldierBombs,
				NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Use the hammer to place the bombs and explode each with one click."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The bombs can use grenades to add."), NULL);
			Buffer.append("\n\n");
		...
		else if (str_comp_nocase(pHelpPage, "scientist") == 0)
		{
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Additionally he can place white holes with the laser rifle. Further information on /help whitehole"), NULL);
		...
		else if (str_comp_nocase(pHelpPage, "police") == 0)
		{
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("And he has a gun, damage is very high.But ammo is scarce"), NULL);
			pSelf->SendMOTD(ClientID, Buffer.buffer());
		}
		else if (str_comp_nocase(pHelpPage, "reviver") == 0)
		{
			Buffer.append("~~ ");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Reviver"), NULL);
			Buffer.append(" ~~\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The Reviver can protect humans with his hammer by giving them health.But can't heal heroes and medics."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He also has a laser-rifle that can freeze zombies."), NULL);
			Buffer.append("\n\n");
		...
		else if (str_comp_nocase(pHelpPage, "biologist") == 0)
		{
			...
			Buffer.append(" ~~\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The Biologist has a shotgun with bouncing bullets and can create a flower-like-laser-trap by shooting with his rifle."), NULL);
		...
		else if (str_comp_nocase(pHelpPage, "looper") == 0)
		{
			...
			Buffer.append(" ~~\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The Looper has a laser wall that slows down zombies and a fast shooting low-range-laser-rifle."), NULL);
			Buffer.append(" \n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He also has a grenades."), NULL);
		...
		else if (str_comp_nocase(pHelpPage, "mercenary") == 0)
		{
			...
			Buffer.append(" ~~\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The Mercenary can fly using his machine gun."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He can coil explosives with his hammer and grenade that can explode to damage infected and self."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_LP(
				Buffer, pLanguage, g_Config.m_InfPoisonDamage,
				_P("He can also throw poison grenades that each deal one damage point.", "He can also throw poison grenades that each deal {int:NumDamagePoints} damage points during two seconds."),
		...
		else if (str_comp_nocase(pHelpPage, "smoker") == 0)
		{
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(
				Buffer, pLanguage, _("He can also inflict {int:DamagePoints} damage points per second by hooking humans."), "DamagePoints", &g_Config.m_InfSmokerHookDamage,
				NULL);
			pSelf->SendMOTD(ClientID, Buffer.buffer());
		}
		else if (str_comp_nocase(pHelpPage, "boomer") == 0)
		{
			...
			Buffer.append(" ~~\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The Boomer explodes when he attacks."), NULL);
			Buffer.append("\n\n");
		...
		else if (str_comp_nocase(pHelpPage, "hunter") == 0)
		{
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He has tripple jump."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He can also inflict 1 damage point per second by hooking humans."), NULL);
		...
		else if (str_comp_nocase(pHelpPage, "bat") == 0)
		{
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He can jump infinitely."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He can also inflict 1 damage point per second by hooking humans."), NULL);
		...
		else if (str_comp_nocase(pHelpPage, "ghost") == 0)
		{
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He is invisible, except if a human is near him, if he takes damage or if he uses his hammer."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He can also inflict 1 damage point per second by hooking humans."), NULL);
		...
		else if (str_comp_nocase(pHelpPage, "spider") == 0)
		{
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("When selecting any gun, his hook enters web mode."), NULL);
			Buffer.append("\n\n");
		...
		else if (str_comp_nocase(pHelpPage, "voodoo") == 0)
		{
			...
			Buffer.append(" ~~\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The Voodoo can infect humans and heal infected with his hammer."), NULL);
			Buffer.append("\n\n");
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("While in Spirit mode he cannot be killed. When his time is up he finally dies."), NULL);
		...
		else if (str_comp_nocase(pHelpPage, "whiteHole") == 0)
		{
			...
			Buffer.append(" ~~\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Receive it by killing at least {int:NumKills} zombies as scientist."),
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Puts humans (if enabled) and zombies into a vulnerable state by pulling them into its center"), NULL);
		...
		else if (str_comp_nocase(pHelpPage, "undead") == 0)
		{
			...
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Instead of dying, he freezes for 10 seconds."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("If an infected heals him, the freeze effect disappears."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He can also inflict 1 damage point per second by hooking humans."), NULL);
		...
		else if (str_comp_nocase(pHelpPage, "freezer") == 0)
		{
			Buffer.append("~~ ");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Freezer"));
			Buffer.append(" ~~\n\n");
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The Freezer can infect humans and heal infected with his hammer."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He can use freeze mine with his hammer."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The freeze mine can freeze humans."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("If some humans in its radius. Humans will be freeze"), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Kill {int:Num} humans can found a freezer mine."), "Num", &g_Config.m_InfFreezeMineMinimalKills, NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("If the Freezer dies, he disappears and is replaced by another class of infected."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He can also inflict {int:Damage} damage point per second by hooking slowly humans."), "Damage", &g_Config.m_InfFreezerHookDamage, NULL);
			pSelf->SendMOTD(ClientID, Buffer.buffer());
		}
		else if (str_comp_nocase(pHelpPage, "nightmare") == 0)
		{
			Buffer.append("~~ ");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Nightmare"));
			Buffer.append(" ~~\n\n");
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The Nightmare can infect humans and heal infected with his hammer."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He is invisible, except if a human is near him, if he takes damage or if he uses his hammer."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("If he is invisible. and a human is near him."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He is visible. The human will in 3 sec nightmare"), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("If humans in nightmare, he can't see any entity else him."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The Nightmare's Hook lenth is 15 blocks, and his gravity is very low."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("He can also inflict {int:Damage} damage point per second by hooking humans."), "Damage", &g_Config.m_InfNightmareHookDamage, NULL);
			pSelf->SendMOTD(ClientID, Buffer.buffer());
		}
		...
		else if (str_comp_nocase(pHelpPage, "endlessHook") == 0)
		{
			Buffer.append("~~ ");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("How to use endless hook"), NULL);
			Buffer.append(" ~~\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("one person has to press F3 to deactivate hook protection, he can be hooked endlessly"), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("the other person has to be hook protected, he hooks the first person to enable this mode"), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("To deactivate endlessHook either toggle hook protection or retract your hook."), NULL);

			pSelf->SendMOTD(ClientID, Buffer.buffer());
		}
		else if (str_comp_nocase(pHelpPage, "turret") == 0)
		{
			Buffer.append("~~ ");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("turret"), NULL);
			Buffer.append(" ~~\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Collect a flag to find a turret"), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("As a hero you can place the turret with your hammer"), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("The turret has a reload and warm up time before it either shoots laser or plasma projectiles"), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("It dissapears after a while and targets maximal 3 zombies at the same time."), NULL);
			Buffer.append("\n\n");
			pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Zombies can destroy the turret by running into it, but it costs them many life points"), NULL);
	...
	if (!pHelpPage)
	{
		...
		pSelf->Server()->Localization()->Format_L(Buffer, pLanguage, _("Available help pages: {str:PageList}"),
												  "PageList", "game, msg, taxi, endlessHook",
												  NULL);
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "help", Buffer.buffer());

		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "help", "engineer, soldier, scientist, sciogist, catapult, police, reviver, medic, hero, ninja, mercenary, sniper, whiteHole");
